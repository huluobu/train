...
//    查看某套餐详情：根据套餐名字找到套餐，调用显示套餐详情的方法
    void showPackageDetail(String packName) throws Exception;

//    添加某套餐的检查项：根据套餐名找到套餐，若已经包括该检查项，则返回false表示不用重复添加，否则给套餐添加一个检查项，并更新总价
    boolean addItem(String packName, String itemName) throws Exception;

//    减少某套餐的检查项：根据套餐名找到套餐，循环该套餐的所有检查项，找到需要删除的检查项，删除，并更新总价
    boolean delItem(String packName, String itemName) throws Exception;

//    修改套某餐的折扣：根据套餐名找到套餐，更新该套餐的折扣，更新总价
    boolean updatePrice(String packName, double discount) throws Exception;

//    新增套餐：若所有套餐列表map中已经包括该套餐名，返回false表示套餐名不允许重复，否则添加一个空的新套餐
    boolean addPackage(String newpackName) throws Exception;
